  ...
  return (
    <div className="rounded-xl bg-zinc-800 p-2 flex flex-col gap-3 my-4">
      <div className="flex justify-between">
        ...
        <span onClick={() => onDelete(habit.id)} className="text-red-500 hover:text-red-400 cursor-pointer ">Delete</span>
      </div>
      <div className="flex gap-2 justify-around ">
        {visibleDates.map((date) => (
          <Button className="flex flex-1 flex-col items-center justify-center gap-0.5 rounded-lg min-h-15 py-2" key={date.toISOString()} disabled={isFuture(date)}>
            <span className="flex flex-col items-center">
              <span className="text-xs opacity-80">{format(date, "EEE")}</span>
              <span className="text-zinc-300 font-medium">{format(date, "d")}</span>
            </span>
          </Button>
        ))}
      </div>
